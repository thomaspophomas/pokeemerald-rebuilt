...
`SOFT` uses `softExpCurve` to map the level delta `pokemonLevel - activeCap`
to an EXP percentage. The supported delta range is `-100..99`; negative values
mean the Pokemon is below the active cap, positive values mean it is above the
cap. `softExpCurve` may be a list of ranges, a map of exact deltas, or a
200-entry percent table ordered from `-100` through `99`. If omitted, the
...
`asset`/`assetKey`, `graphicsRevision`, and `graphicsInfoSymbol` are optional.
`graphicsInfoSymbol` lets a compiled mod provide a direct
`ObjectEventGraphicsInfo` while the `graphicsId` remains a stable fallback ID.
