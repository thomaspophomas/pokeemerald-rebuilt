...
static bool8 DefinitionIsEmptyDefault(const struct ModPokemonDataDefinition *definition)
{
    return definition->key == NULL
        ...
        && definition->flags == 0
        && definition->level_up_move_count == 0
        && definition->evolution_count == 0;
}
...
static void ApplyInfoOverride(struct SpeciesInfo *species_info_to_update, const struct ModSpeciesInfoOverride *species_info_override, u16 override_flags)
{
    ...
    {
        species_info_to_update->baseHP = species_info_override->base_hp;
        species_info_to_update->baseAttack = species_info_override->base_attack;
        species_info_to_update->baseDefense = species_info_override->base_defense;
        species_info_to_update->baseSpeed = species_info_override->base_speed;
        species_info_to_update->baseSpAttack = species_info_override->base_sp_attack;
        species_info_to_update->baseSpDefense = species_info_override->base_sp_defense;
    }
    ...
    if (override_flags & MOD_POKEMON_OVERRIDE_CATCH_RATE)
        species_info_to_update->catchRate = species_info_override->catch_rate;
    if (override_flags & MOD_POKEMON_OVERRIDE_EXP_YIELD)
        species_info_to_update->expYield = species_info_override->exp_yield;
    if (override_flags & MOD_POKEMON_OVERRIDE_EV_YIELD)
    {
        species_info_to_update->evYield_HP = species_info_override->ev_yields[0];
        species_info_to_update->evYield_Attack = species_info_override->ev_yields[1];
        species_info_to_update->evYield_Defense = species_info_override->ev_yields[2];
        species_info_to_update->evYield_Speed = species_info_override->ev_yields[3];
        species_info_to_update->evYield_SpAttack = species_info_override->ev_yields[4];
        species_info_to_update->evYield_SpDefense = species_info_override->ev_yields[5];
    }
    if (override_flags & MOD_POKEMON_OVERRIDE_ITEMS)
    {
        species_info_to_update->itemCommon = species_info_override->item_common;
        species_info_to_update->itemRare = species_info_override->item_rare;
    }
    if (override_flags & MOD_POKEMON_OVERRIDE_GENDER)
        species_info_to_update->genderRatio = species_info_override->gender_ratio;
    if (override_flags & MOD_POKEMON_OVERRIDE_EGG_CYCLES)
        species_info_to_update->eggCycles = species_info_override->egg_cycles;
    ...
    if (override_flags & MOD_POKEMON_OVERRIDE_GROWTH_RATE)
        species_info_to_update->growthRate = species_info_override->growth_rate;
    if (override_flags & MOD_POKEMON_OVERRIDE_EGG_GROUPS)
    {
        species_info_to_update->eggGroups[0] = species_info_override->egg_groups[0];
        species_info_to_update->eggGroups[1] = species_info_override->egg_groups[1];
    }
    ...
    if (override_flags & MOD_POKEMON_OVERRIDE_SAFARI_FLEE)
        species_info_to_update->safariZoneFleeRate = species_info_override->safari_zone_flee_rate;
    if (override_flags & MOD_POKEMON_OVERRIDE_BODY_COLOR)
    {
        species_info_to_update->bodyColor = species_info_override->body_color;
        species_info_to_update->noFlip = species_info_override->no_flip;
    }
}
...
bool8 PokemonDataApi_IsDefinitionValid(const struct ModPokemonDataDefinition *definition, bool8 allow_empty_default)
{
    ...
    if (definition->species == SPECIES_NONE || definition->species >= NUM_SPECIES)
        return FALSE;
    if (definition->level_up_move_count > MOD_POKEMON_MAX_LEVEL_UP_MOVES)
        return FALSE;
    if (definition->evolution_count > MOD_POKEMON_MAX_EVOLUTIONS)
        return FALSE;
    for (level_up_move_index = 0; level_up_move_index < definition->level_up_move_count; level_up_move_index++)
    {
        if (definition->level_up_moves[level_up_move_index].level == 0 || definition->level_up_moves[level_up_move_index].level > MAX_LEVEL)
            return FALSE;
        if (definition->level_up_moves[level_up_move_index].move == MOVE_NONE || definition->level_up_moves[level_up_move_index].move >= MOVES_COUNT)
            return FALSE;
    }
    for (evolution_index = 0; evolution_index < definition->evolution_count; evolution_index++)
    {
...
const u16 *PokemonDataApi_GetLevelUpLearnset(u16 species)
{
    ...
    for (level_up_move_index = 0; level_up_move_index < definition->level_up_move_count; level_up_move_index++)
        sEffectiveLevelUpLearnset[level_up_move_index] = (definition->level_up_moves[level_up_move_index].level << 9) | definition->level_up_moves[level_up_move_index].move;
    sEffectiveLevelUpLearnset[definition->level_up_move_count] = LEVEL_UP_END;
